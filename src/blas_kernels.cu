...
__global__ void permute_conf_data_kernel(const float* conf_data,
                         const int num,
...
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if(id >= num * num_classes * num_preds_per_class) return;
	
	const int i = id / (num_classes * num_preds_per_class);
	int temp = id % (num_classes * num_preds_per_class);
	const int c = temp % num_preds_per_class;
	const int p = temp / num_preds_per_class;
	const int src_index = (i * num_preds_per_class + c) * num_classes + p;
	conf_preds[id] = conf_data[src_index];
}



void permute_conf_data_gpu(const float *conf_data, const int num, const int num_preds_per_class, const int num_classes, float* conf_preds)
{
	
	int num_threads = num * num_preds_per_class * num_classes;
	
	permute_conf_data_kernel<<<cuda_gridsize(num_threads), BLOCK>>>(conf_data, num, num_preds_per_class, num_classes, conf_preds);
	check_error(cudaPeekAtLastError());
}


__device__ void clip_box_device(const float *bbox,float *clipBox)
{
	float tmp = bbox[0]>1.f ? 1.f : bbox[0];
    clipBox[0] = tmp > 1e-6 ? tmp : 0.f;
    tmp = bbox[1]>1.f ? 1.f : bbox[1];
    clipBox[1] = tmp > 1e-6 ? tmp : 0.f;
    tmp = bbox[2]>1.f ? 1.f : bbox[2];
    clipBox[2] = tmp > 1e-6 ? tmp : 0.f;
    tmp = bbox[3]>1.f ? 1.f : bbox[3];
    clipBox[3] = tmp > 1e-6 ? tmp : 0.f;
}


__device__ void decode_box_device(const float *prior_bbox,
                    ...
                    float *decode_bbox)
{
    if (code_type == PriorBoxParameter_CodeType_CORNER) {
        ...
        prior_width = prior_width > 1e-6 ? prior_width : 0.f;
        
        float prior_height = prior_bbox[3] - prior_bbox[1];
        prior_height = prior_height > 1e-6 ? prior_height : 0.f;
        
        float prior_center_x = (prior_bbox[0] + prior_bbox[2]) * 0.5f;
        float prior_center_y = (prior_bbox[1] + prior_bbox[3]) * 0.5f;
        ...
        decode_bbox[0] = decode_bbox_center_x - decode_bbox_width * 0.5f;
        decode_bbox[1] = decode_bbox_center_y - decode_bbox_height * 0.5f;
        decode_bbox[2] = decode_bbox_center_x + decode_bbox_width * 0.5f;
        decode_bbox[3] = decode_bbox_center_y + decode_bbox_height * 0.5f;
    ...
    if (clip_bbox) {
        clip_box_device(decode_bbox, decode_bbox);
    }
    
}


__global__ void decode_box_data_kernel(const float *loc_data, const float *priorbox_data, const float *variance_data, const int num_priors, const int num, const int share_location, 
                                  const int background_label_id, const int code_type, const int variance_encoded_in_target, const int clip, float *all_decode_box_data)
{
	int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	int i = id / num_priors;
	int j = id % num_priors;
	...
	decode_box_device(priorbox_data + 4 * j, variance_data + i * 4, code_type, variance_encoded_in_target, clip, loc_data + offset, all_decode_box_data + offset);
	
}


void decode_all_box_gpu(const float *loc_data,
                         const float *priorbox_data,
                         ...
                         const int variance_encoded_in_target,
                         const int clip,
                         float* all_decode_box_data)
{
    
    const float *prior_variances = priorbox_data + num_priors * 4;
    
    int num_threads = num * num_priors;
    
    decode_box_data_kernel<<<cuda_gridsize(num_threads), BLOCK>>>(loc_data, priorbox_data, prior_variances, num_priors, num, share_location, background_label_id, code_type, variance_encoded_in_target, clip, all_decode_box_data);
	check_error(cudaPeekAtLastError());
}
