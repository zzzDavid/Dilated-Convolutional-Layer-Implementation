...
__global__ void activate_array_relu_with_bias_kernel(float * __restrict__ output, const float * __restrict__ biases, int batch, int n, int size)
{
    ...
    index /= n;
    int k = index;
    int target_loc = (k*n+j)*size + i;
    float data = output[target_loc];
    data += biases[j];
    output[target_loc] = (data > 0) * data;
...
extern "C" void activate_array_relu_with_bias_gpu(float *output, float *biases, int batch, int n, int size)
{
	int num = n*size*batch;
	activate_array_relu_with_bias_kernel<<<cuda_gridsize(num), BLOCK>>>(output, biases, batch, n, size);
    check_error(cudaPeekAtLastError());
}
