                           ...
                           uint32_t width,       uint32_t height)
{
    ...
    dstImage[y * width + x]     = make_float3(red[0], green[0], blue[0]);
    dstImage[y * width + x + 1] = make_float3(red[1], green[1], blue[1]);
#else
    //printf("cuda thread %i %i  %i %i \n", x, y, width, height);
        
    dstImage[y * width + x]     = make_float3(1.0f, 0.0f, 0.0f);
    dstImage[y * width + x + 1] = make_float3(1.0f, 0.0f, 0.0f);
#endif
}
// cudaNV12ToRGB
