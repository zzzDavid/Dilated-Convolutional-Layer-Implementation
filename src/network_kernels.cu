...
/*
    if(!net.input_gpu)
    {
        printf("cuda_make_array(net.input_gpu, net.input, x_size: %d);\n", x_size);
        net.input_gpu = cuda_make_array(net.input, x_size);
    } else {
        printf("cuda_push_array(net.input_gpu, net.input, x_size: %d);\n", x_size);
    ...
    if(!net.truth_gpu)
    {
        printf("cuda_make_array(net.truth_gpu, net.truth, y_size: %d);\n", y_size);
        net.truth_gpu = cuda_make_array(net.truth, y_size);
    } else {
        printf("cuda_push_array(net.truth_gpu, net.truth, y_size: %d);\n", y_size);
...
/*
    if(l.type == DETECTION_OUTPUT) {
    //    cuda_pull_array(l.kept_sli_gpu, l.kept_sli, l.total_num * 3);
    //    cuda_pull_array(l.all_box_data_gpu, l.all_decode_bboxes, l.batch * l.num_priors * 4);
    } else {
		cuda_pull_array(l.output_gpu, l.output, l.outputs*l.batch);
	}
*/
...
float *network_predict_with_gpu_input(network net)
{
    cuda_set_device(net.gpu_index);
    //~ cuda_push_array(net.input_gpu, input, net.inputs*net.batch);
    net.truth = 0;
